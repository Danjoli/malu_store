                    ...
                    <tr class="transition hover:bg-[#fdf8f6]">
                        ...
                        <td class="p-4 text-right">
                            <a href="{{ route('admin.categories.show', $categorie) }}"
                                class="text-xs font-bold text-[#625956] hover:text-[#b85d70]">Ver</a>
                            ...
                            <form action="{{ route('admin.categories.destroy', $categorie) }}" method="POST"
                                class="ml-3 inline">@csrf @method('DELETE')<button type="submit"
                                    ...
                                    onclick="return confirm('Tem certeza que deseja excluir esta categoria?')">Excluir</button>
                            </form>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="3" class="p-12 text-center text-[#746b68]">Nenhuma categoria cadastrada ainda.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
