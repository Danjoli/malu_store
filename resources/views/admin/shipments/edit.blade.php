...
@section('content')
    <div class="mx-auto max-w-2xl">
        <a
            ...
            class="text-sm font-semibold text-[#b85d70] hover:text-[#9f4c5e]"
        >
            ← Voltar para envios
        </a>

        <p class="mt-6 text-xs font-bold uppercase tracking-[0.18em] text-[#c96f82]">
            Logística
        </p>
        ...
        <p class="mt-1 text-sm text-[#746b68]">
            Pedido #{{ $shipment->order->id }}
        </p>

        <form
            action="{{ route('admin.shipments.update', $shipment) }}"
            method="POST"
        ...
        >
            ...
            <div class="flex items-center justify-between border-t border-[#f0e5e1] pt-5">
                <a
                    href="{{ route('admin.shipments.index') }}"
                ...
                <button
                    type="submit"
                    class="rounded-xl bg-[#cf7184] px-4 py-3 text-sm font-bold text-white hover:bg-[#b85d70]"
                >
                    Atualizar envio
                </button>
            </div>
