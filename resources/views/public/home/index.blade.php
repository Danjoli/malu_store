...
@section('content')
    {{-- Banner principal da coleção em destaque. --}}
    <section class="border-b border-[#eee6e4] bg-[#f8eee9]">
        <div class="relative min-h-[440px] overflow-hidden sm:min-h-[410px] md:min-h-[450px]">
            ...
            <div
                class="absolute inset-0 bg-gradient-to-r from-[#f8eee9] via-[#f8eee9]/90 to-[#f8eee9]/10 sm:via-[#f8eee9]/78 sm:to-transparent">
            </div>

            <div
                class="store-container home-container relative z-10 flex min-h-[440px] items-center py-12 sm:min-h-[410px] md:min-h-[450px]">
    ...
    {{-- Categorias disponíveis no catálogo exibido na home. --}}
    @php
            ...
            <div class="grid grid-cols-3 gap-x-3 gap-y-6 sm:grid-cols-6 sm:gap-4 md:gap-5">
                @foreach ($categories as $category)
                    ...
                    <a href="{{ route('home', ['category' => $category->slug]) }}" class="group text-center">
                        <div
                            class="mx-auto aspect-square w-full max-w-24 overflow-hidden rounded-full border-4 border-[#f8eee9] bg-[#f1e4de] shadow-sm transition duration-300 group-hover:-translate-y-1 group-hover:border-[#f4cfca] group-hover:shadow-lg sm:max-w-32">
                            @if ($categoryImage)
                                <img src="{{ asset('storage/products/' . $categoryImage->image) }}"
                                    alt="{{ $category->name }}"
    ...
    <div id="produtos" class="store-container home-container py-14">
        <div class="mb-8 flex items-center gap-2 sm:gap-4">
            <span class="h-px flex-1 bg-[#eadfdd]"></span>
            <div class="text-center">
                <h2 class="store-title text-2xl sm:text-3xl">Novidades</h2>
            </div>
        ...
        <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 lg:gap-6">
            ...
            @empty
                <div
                    class="col-span-full rounded-md border border-dashed border-[#e8d9d6] py-16 text-center text-stone-500">
    ...
    {{-- Benefícios que reforçam a compra na loja. --}}
    ...
    {{-- Galeria social baseada nas imagens disponíveis do catálogo. --}}
    @php
        $galleryImages = $products->flatMap(fn ($product) => $product->images)->take(4);
    @endphp
        ...
        <section class="store-container home-container py-14">
            <div class="grid overflow-hidden rounded-md border border-[#eee6e4] md:grid-cols-[.9fr_2.1fr]">
                <div class="bg-[#fff1ef] p-8">
                    <p class="store-kicker text-[#bd5564]">#malustore</p>
                    <h2 class="store-title mt-3 text-2xl">Nos marque e apareça por aqui!</h2>
                ...
                <div class="grid grid-cols-4 gap-1.5 bg-white p-1.5">
                    @foreach ($galleryImages as $image)
                        <img src="{{ asset('storage/products/' . $image->image) }}" alt="Malu Store"
