        ...
        <section class="w-full overflow-hidden rounded-2xl border border-[#eaded9] bg-white shadow-[0_12px_34px_rgba(76,50,47,0.08)]">
            ...
            <form method="POST" action="/login" class="space-y-5 px-6 py-7 sm:px-8">
                ...
                <div>
                    ...
                    <input id="email" type="email" name="email" value="{{ old('email') }}" autocomplete="email" required autofocus placeholder="voce@email.com" class="w-full rounded-xl border border-[#ded4d0] px-4 py-3 text-sm outline-none transition focus:border-[#cf7184] focus:ring-4 focus:ring-[#f7dce2]">
                </div>

                <div>
                    <label for="password" class="mb-2 block text-sm font-semibold text-[#443d3b]">Senha</label>
                    <input id="password" type="password" name="password" autocomplete="current-password" required placeholder="Sua senha" class="w-full rounded-xl border border-[#ded4d0] px-4 py-3 text-sm outline-none transition focus:border-[#cf7184] focus:ring-4 focus:ring-[#f7dce2]">
                </div>

                <button class="w-full rounded-xl bg-[#cf7184] py-3.5 text-sm font-bold text-white transition hover:bg-[#b85d70]">Entrar</button>

                <p class="text-center text-sm text-[#746b68]">Ainda não tem conta? <a href="{{ route('register') }}" class="font-bold text-[#b85d70] hover:text-[#9f4c5e]">Criar conta</a></p>
            </form>
        </section>
    </div>
@endsection
