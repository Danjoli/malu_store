                        ...
                        <tr class="border-t border-[#eee6e4] transition hover:bg-[#fffaf9]">
                            ...
                            <td class="p-4">
                                @if ($order->status == 'pending')
                                    <span
                                        class="rounded-full bg-amber-50 px-2.5 py-1 text-xs font-semibold text-amber-700">Aguardando
                                        pagamento</span>
                                ...
                                @elseif($order->status == 'paid')
                                    <span
                                        class="rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-semibold text-emerald-700">Pago</span>
                                @elseif($order->status == 'cancelled')
                                    <span
                                        class="rounded-full bg-red-50 px-2.5 py-1 text-xs font-semibold text-red-600">Cancelado</span>
                                @else
                                    <span
                                        class="rounded-full bg-stone-100 px-2.5 py-1 text-xs font-semibold text-stone-700">{{ $order->status === 'failed' ? 'Pagamento recusado' : ($order->status === 'shipped' ? 'Pedido enviado' : ($order->status === 'delivered' ? 'Pedido entregue' : 'Em processamento')) }}</span>
                                @endif
                            </td>
                        ...
                        <tr>
                            <td colspan="6" class="p-12 text-center text-stone-500">
                                ...
                                <a href="{{ route('catalog.index') }}"
                                    class="store-button store-button-primary mt-5">Conhecer a coleção</a>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
